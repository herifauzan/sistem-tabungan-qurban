...
// ⚡ Bolt: Cache in-flight getRows requests (Promise Coalescing)
// This prevents duplicate concurrent requests to the Google Sheets API
// when multiple components or API routes request the same sheet simultaneously,
// reducing N+1 read bottlenecks and API quota consumption.
...
export async function getRows(sheetName: string): Promise<string[][]> {
  // If there's already a request in flight for this sheet, reuse that promise
  ...
  const promise = (async () => {
    try {
      ...
      // Clear the promise from the cache once it settles,
      // so future requests get fresh data.
